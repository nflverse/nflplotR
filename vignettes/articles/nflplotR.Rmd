...
* the axis labels are replaced with the logos by setting `axis.text.x = nflplotR::element_nfl_logo()` in the ggplot2 theme call. 
...
The R package gt is becoming increasingly popular for creating aesthetically pleasing tables. nflplotR supports rendering of team logos, team wordmarks, and player headshots in gt tables similar to ggplot2. 
...
It is also possible to render headshots through gsis IDs.
...
df |>
  gt() |> 
  gt_nfl_headshots(columns = gt::everything(), height = 50)
```
