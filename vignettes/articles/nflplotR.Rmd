...
nflplotR is a normal R package but since it is still quite new, you can't download it from CRAN yet. The plan is to release the package on CRAN and the code base is ready for it, however it depends on another package that needs to be updated on CRAN first. Until then, you can install nflplotR from GitHub with
...
Trying to install nflplotR from CRAN using `install.packages()` results in an uninformative and rather confusing error message (see below) suggesting that you need to update R which is not the case!

```{r, eval=FALSE}
install.packages("nflplotR")
#>Warning in install.packages :
#>  package ‘nflplotR’ is not available for this version of R
```
...
ggplot2::ggplot(qbs, aes(x = reorder(team, -qb_epa), y = qb_epa)) +
  ...
  # this line triggers the download of the logo images and calls a ggplot 
